...
map('n', 'gx', require('bsaada.user.open-url').open_url_under_cursor, { remap = false, desc = 'Open url under cursor' })
...
vim.api.nvim_create_user_command('Whereami', function()
  ...
  local emoji = require('bsaada.user.utils').country_os_to_emoji(iso)
  if not emoji then
    emoji = '🌎'
  end
...
-- Grep --
----------
require('bsaada.user.grep').setup()
...
-- Search and Replace --
------------------------
vim.cmd('source ' .. vim.fn.stdpath 'config' .. '/lua/bsaada/user/search-replace.vim')
