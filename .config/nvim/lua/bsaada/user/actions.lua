...
return {
  ...
  ['[Terraform] Remove terragrunt files'] = function()
    ...
    local terraform_repo = vim.trim(vim.fn.FugitiveExecute({ 'rev-parse', '--show-toplevel' }).stdout or '')
    if terraform_repo == '' then
      pretty_print('Not inside a git repository', 'Terraform', '', vim.log.levels.ERROR)
      return
    end
    ...
    if #to_delete == 0 then
      pretty_print('No terragrunt-generated files found', 'Terraform')
      return
    end

    vim.ui.select({ 'Yes', 'No' }, { prompt = string.format('Delete %d terragrunt-generated file(s)?', #to_delete) }, function(choice)
      if choice ~= 'Yes' then
        return
      end
  ...
  ['Autocommand to reload the lua file nvim'] = function()
    if vim.api.nvim_get_option_value('filetype', { buf = 0 }) ~= 'lua' then
      pretty_print('Filetype is not lua!', 'Reload', [[🖥️]], vim.log.levels.ERROR)
      return
    end
...
vim.keymap.set('n', 'bla', function()
  ...
  ['Save current buffer as temp'] = function()
    local function save_as_temp(filetype)
      -- write a copy under a new tempname and switch the current window to
      -- editing it (a bare `:edit` here would just reload the untouched
      -- original file, discarding the point of "save as temp").
      local tmp = vim.fn.tempname() .. '.' .. filetype
      vim.cmd('write ' .. tmp)
      vim.cmd.edit(tmp)
    end
  ...
  ['[YAML] Add YAML Schema Modeline'] = function()
    require('bsaada.user.additional-schemas').init()
  end,
