          ...
          actions = {
            ['default'] = {
              fn = function(selected, opts)
                ...
                require('bsaada.user.git').reload_fugitive_index()
              end,
              header = 'switch',
            },
            ...
            ['ctrl-s'] = {
              fn = function(selected)
                ...
                require('bsaada.user.git').checkout(branch)
              end,
              reload = false,
              header = 'checkout',
            },
              ...
              fn = function(selected)
                ...
                vim.defer_fn(function()
                  vim.ui.input({ prompt = 'Rename branch: ', default = branch }, function(new_name)
                    ...
                    local _, ret, stderr = require('bsaada.user.utils').get_os_command_output({ 'git', 'branch', '-m', branch, new_name }, toplevel)
                    if ret == 0 then
            ...
            ['ctrl-x'] = {
              fn = function(selected)
                ...
                vim.ui.select({ 'Yes', 'No' }, { prompt = 'Are you sure you want to delete the branch ' .. branch .. '?' }, function(yes_or_no)
                  ...
                  -- Delete the branch
                  local toplevel = vim.trim(vim.system({ 'git', 'rev-parse', '--show-toplevel' }, { text = true }):wait().stdout)
                  local _, ret, stderr = require('bsaada.user.utils').get_os_command_output({ 'git', 'branch', '-D', branch }, toplevel)
                  if ret == 0 then
                    ...
                    vim.ui.select({ 'Yes', 'No' }, { prompt = 'Delete also from remote?' }, function(yes_or_no_remote)
                      ...
                      local _, ret_remote, stderr_remote =
                        require('bsaada.user.utils').get_os_command_output({ 'git', 'push', 'origin', '--delete', branch }, toplevel)
